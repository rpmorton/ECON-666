...
options  =  optimset('GradObj','off','LargeScale','off','Display','iter','TolFun',1e-16,'TolX',1e-16,'Diagnostics','on','MaxFunEvals',200000,'MaxIter',1000); 
[estimate_cutter_count] = fminunc(@(x)lognormleastsquares(x,cutter_count,quantile),x_hat,options);



%%
%Fit Lognormal: cutter piece rate
...
options  =  optimset('GradObj','off','LargeScale','off','Display','iter','TolFun',1e-16,'TolX',1e-16,'Diagnostics','on','MaxFunEvals',200000,'MaxIter',1000); 
[estimate_cutter_piece] = fminunc(@(x)lognormleastsquares(x,cutter_piece_rate,quantile),x_hat,options);
