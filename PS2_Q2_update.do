...
*Generate Output
g re = rnormal(0,sqrt(8))
g re_first = re if unit_num == 1
bys village: egen re_village = max(re_first)
egen mean_re_village = mean(re_first)
drop re re_first

*g output = re_village + rnormal(0,sqrt(25-8))
g random_norm = rnormal(10,sqrt(25-8))
egen pre_cov_rand = sum(random_norm * re_village)
egen mean_random_norm = mean(random_norm)
g cov_random = (1/3000)*pre_cov_rand - mean_random_norm * mean_re_village
g output = re_village + random_norm + rnormal(0,sqrt(2 * cov_random))

loneway output village
